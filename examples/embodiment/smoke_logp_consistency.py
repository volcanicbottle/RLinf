...
def _run():
    ...
    print("\n=== Conclusion ===")
    print("SCOPE: this script puts both calls on the SAME policy instance with the")
    print("SAME forward path. It deliberately bypasses what the real training pipeline")
    print("crosses between prev_logp and logp_new:")
    print("  - rollout backend (HuggingFace) vs actor backend (FSDP) — different")
    print("    forward implementations, different numerical ordering")
    print("  - cross-Ray-process boundary + periodic weight_syncer (possible lag)")
    print("  - whether `recompute_logprobs` actually re-runs in the sync embodied")
    print("    runner (read fsdp_actor_worker.py:1400 — sync path skips inference_step)")
    print()
    if diff_A.mean().item() < THRESH and diff_B.mean().item() < THRESH:
        print("PASS (in-scope): adapter math is per-element consistent in bf16. This")
        print("  rules out dropout/mode/chain-index/σ-schedule/get_logprob_norm-formula")
        print("  bugs. It does NOT prove training-time prev_logp matches logp_new — the")
        print("  HF↔FSDP path split + recompute-no-op are the leading suspects for the")
        print("  observed approx_kl≈-0.25 (smoke is blind to both).")
        print()
        print("  To pin down the actual training-time source, two cheap experiments:")
        print("    1. Log approx_kl at training step 1 BEFORE any gradient update — if")
        print("       it's already -0.25, the bias is path/recompute, not policy drift.")
        print("    2. Take one rollout batch, score it once via the rollout HF path and")
        print("       once via the actor FSDP path; the per-element diff is the bias.")
    elif diff_A.mean().item() < THRESH and diff_B.mean().item() >= THRESH:
        print("PARTIAL (in-scope): eval/eval is consistent but eval/train diverges in")
        print("  the same instance. Some train-time op (dropout/BN/RNG hook) shifts the")
        print("  forward output. Fix: enforce model.eval() at get_log_prob_value or audit")
        print("  SmolVLM2 submodules for train-time stochasticity. Still says nothing about")
        print("  the HF↔FSDP path split in training (separate experiment needed).")
    else:
        print("FAIL (in-scope): per-element bug in sample/rescore paths of the same")
        print(f"  instance. max diff = {max(diff_A.max().item(), diff_B.max().item()):.6f}")
        print("  Investigate before trusting Phase 5: chains storage layout, denoise_inds")
        print("  selection, sample_mean_var_val determinism. (The training-time HF↔FSDP")
        print("  split is downstream of this and only worth analysing once this is fixed.)")
